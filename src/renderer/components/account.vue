<template>
  <div id="accountWrapper" v-if="isRouterAlive">
    ...
    <img id="logoSmall" src="~@/assets/ann.png" alt="electron-vue">
      ...
      <div id="reflashButton" v-if="accountInfoShow">
        <el-button size="small" type="success" icon="el-icon-refresh" circle @click="reflashTable"></el-button>
      </div>
      ...
      <div id="accountButtonBIG" v-if="accountList.length == 0">
        <el-button v-if="cardShow" type="success" @click="goCreatAccount"  icon="el-icon-plus" round>create account</el-button>
      ...
      <div id="cards" v-if="cardShow" v-loading="cardLoading">
        <el-card id="box-card" :key="account.account_name" v-for="account in accountList" shadow="always" @click="accountInfo(account)">
            ...
            empty-text="empty">
              <el-table-column fixed prop="txHash" label="TRANCACTIONS" width="210" sortable style="color:#2d3e50" :show-overflow-tooltip="false">
                <template slot-scope="scope">
                  <el-button type="text" @click="txDetail(scope.row.txHash)">{{ scope.row.txHash.substring(0,10)+'...'+scope.row.txHash.substring(10,20) }}</el-button>
                </template>
              </el-table-column>
              <el-table-column prop="cStatus" label="STATUS" width="95" style="color:#2d3e50" :show-overflow-tooltip="false">
                <template slot-scope="scope">
                    <div slot="reference" class="name-wrapper">
                      <el-tag size="medium" type="success" v-if="scope.row.cStatus=='success'">{{ scope.row.cStatus }}</el-tag>
                      <el-tag size="medium" type="danger" v-if="scope.row.cStatus=='failed'">{{ scope.row.cStatus }}</el-tag>
                      <el-tag size="medium" type="warning" v-if="scope.row.cStatus=='tx not found'">{{ scope.row.cStatus }}</el-tag>
                      <el-tag size="medium" v-if="scope.row.cStatus=='pending'">{{ scope.row.cStatus }}</el-tag>
                    </div>
                </template>
              </el-table-column>
              <el-table-column prop="cFrom" label="FROM" width="210" sortable :show-overflow-tooltip="false">
              </el-table-column>
              <el-table-column prop="cTo" label="TO" width="210" sortable :show-overflow-tooltip="false">
              </el-table-column>
              <el-table-column prop="ConfirmTime" label="TIMESTAMP" width="120" sortable :show-overflow-tooltip="false">
              </el-table-column>
      ...
      <el-dialog
        ...
        <div id="TxDetail">
          ...
          <div id="TxStatus">
            ...
            <el-alert
              center
              v-if="txOBJ.cStatus=='tx not found'"
        ...
        <el-dialog title="TRANSFER DETAILS" :visible.sync="transactionFromShow" :width="widthP">
          <el-form>
            ...
            <el-form-item label="recipient address" >
              <el-input v-model="tx.to" placeholder="the recipient address" :autofocus=true :clearable=true></el-input>
            </el-form-item>
          ...
          <div slot="footer" class="dialog-footer">
            <el-button type="danger" @click="transactionFromShow = false;tx = {passwoed:''}" icon="el-icon-close" plain> Cancel</el-button>
          ...
          <el-dialog width="60%" title="SIGNER" :visible.sync="enterPassShow" append-to-body>
            ...
            <el-button id="signer-button-cancel" type="danger" @click="enterPassShow=false;transactionFromShow = false;tx={password: ''}" icon="el-icon-close" plain style="margin-top: 20px;"> Cancel</el-button>
      ...
      <div id="editForm">
        ...
        <el-dialog title="EDIT METADATA" :visible.sync="editFromShow" :width="widthP">
          ...
          <div slot="footer" class="dialog-footer">
            <el-button type="danger" @click="editFromShow = false;editData={}" icon="el-icon-close" plain> Cancel</el-button>
            <el-button type="success" @click="editAccount(editData);editData={}" icon="el-icon-check"> Save</el-button>
          </div>
        </el-dialog>
      </div>
      <div id="passwordForm">
...
export default {
  ...
  methods: {
    tableRowStyle ({ row, rowIndex }) {
      return 'color:#2d3e50; background: #e4f5f2;'
    },
    ...
    txDetail (txHash) {
      ...
      var sql = 'SELECT * from txHistory where txHash == "' + txHash + '"'
      console.log(sql)
      sqlite.query(sql).then((data) => {
        console.log(data)
        this.txOBJ = data.data[0]
      }).then().catch((err) => {
        console.log(err)
      })
    },
    ...
    toEnterPass () {
      if (this.tx.to === '' || this.tx.to === undefined || this.tx.amount === '' || this.tx.amount === undefined) {
        this.$message({
          message: 'recipient address & amount can not be null',
          type: 'error'
        })
      } else {
